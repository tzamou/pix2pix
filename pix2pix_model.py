...
class Pix2pix:
    def __init__(self,model_name=None,config=None):
        ...
        self.sketchdata, self.realdata =DataLoader().load_data(metrics='[-1,1]')
    ...
    def train(self):
        ...
        for i in range(self.epochs):
            ...
            idx = np.random.randint(0, self.sketchdata.shape[0], self.batchsize)#;print(idx)
            # print(i,idx)
            ...
            if (i + 1) % 500 == 0 or i == 0 or (i+1 <= 3000 and (i+1)%40==0):
                ...
                idx = np.random.randint(0, self.sketchdata.shape[0], self.batchsize)
                fake_images = self.genModel.predict(self.sketchdata[idx])
                fake_images = ((fake_images + 1) * 127.5).astype(np.uint8)
                print(np.max(fake_images),np.min(fake_images))
    ...
    def finish_training(self):
        ...
        gloss = np.load(f'{path.LOG_LOSSPATH}/gloss.npy', allow_pickle=True)
        dloss = np.load(f'{path.LOG_LOSSPATH}/dloss.npy', allow_pickle=True)
        plt.clf()
        plt.title('Loss')
        ...
        plt.legend(loc='best')
        plt.savefig(f"{path.LOG_LOSSPATH}/Generated MAE Loss.png")

    def predict(self, num_images=3):
        test_sketchdata, test_realdata = DataLoader().load_testing_data(metrics='[-1,1]')
