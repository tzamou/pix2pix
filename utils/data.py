...
class DataLoader:
    ...
    def load_data(self,metrics='[0,1]'):
        ...
        self.sketchdata = self.__loadimg(sketchdatapath[0],0)#sketchimg.reshape(1, 256, 256, 3)
        self.realdata = self.__loadimg(realdatapath[0],0)#realimg.reshape(1, 256, 256, 3)
        for index in range(1,len(realdatapath)):
            self.sketchdata = np.concatenate([self.sketchdata,self.__loadimg(sketchdatapath[index],index)],axis=0)
            self.realdata = np.concatenate([self.realdata, self.__loadimg(realdatapath[index],index)], axis=0)
            p = int(30*index/400)
